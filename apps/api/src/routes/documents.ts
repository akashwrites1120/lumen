...
import { IdempotencyConflictError, runIdempotent } from "../lib/idempotency.js";
  ...
  app.post("/v1/projects/:id/documents", async (req, reply) => {
    ...
    try {
      const result = await runIdempotent(
        ...
        `upload:${user.organizationId}:${id}`,
        async () => uploadDocument(ctx, user, id, req)
      );
...
type UploadResult = { statusCode: number; body: unknown };
...
async function uploadDocument(
  ...
  if (!accepted) {
    return {
      statusCode: 415,
      body: { error: "unsupported_media_type", detail: "Accepted formats: EPUB, DOCX, PDF." },
    };
  }
  ...
  if (!scan.clean) {
    ...
    return { statusCode: 422, body: { error: "malware_detected", engine: scan.engine } };
  }
  ...
  return { statusCode: 202, body: { document: doc } };
}
